...
MAX_EXAMPLES_TO_SHOW = 10  # Сколько примеров ошибок каждого типа показать
...
try:
    ...
    dev_docs = list(db.get_docs(nlp.vocab))  # Получаем эталонные Doc объекты
except Exception as e:
...
false_positives = []
false_negatives = []
boundary_errors = []  # Предсказание пересекается с эталоном, но границы не совпадают

examples = []
for gold_doc in dev_docs:
    if not gold_doc.text.strip():  # Пропускаем пустые документы, если вдруг есть
        continue
    # Создаем объект Example для сравнения
    pred_doc = nlp(gold_doc.text)
    examples.append(Example(pred_doc, gold_doc))

    # Получаем множества спанов для удобного сравнения
    gold_spans = set(
    ...
    # Ищем False Positives (есть в предсказаниях, нет в эталоне)
    for start, end, label in pred_spans:
        if (start, end, label) not in gold_spans:
            # Проверим, нет ли пересечения с каким-либо эталонным спаном (это будет boundary error)
            is_boundary = False
    ...
    # Ищем False Negatives (есть в эталоне, нет в предсказаниях)
    for start, end, label in gold_spans:
        if (start, end, label) not in pred_spans:
            # Проверим, нет ли пересечения с каким-либо предсказанным спаном (это будет boundary error)
            is_boundary = False
            for p_start, p_end, p_label in pred_spans:
                if label == p_label and max(start, p_start) < min(end, p_end):
                    is_boundary = True
                    # Найдем предсказанный спан для вывода
                    ...
                    break  # Переходим к следующему эталонному спану
            if not is_boundary:
...
# Дополнительно: можно вывести общий отчет Scorer, чтобы убедиться в цифрах
